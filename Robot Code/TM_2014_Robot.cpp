	...
	//		else if (!isActive(lockingLS) && launcherReleased())
		...
		void turnRobot(float speed) {
			mecDrive(0.0,0.0,speed);
		}
		void teleopShooter() {
...
//		}
		float getTargetPosition() {
			return coordinatesTable->GetNumber("Target Position");
		}
		bool targetDetected() {
			return (getTargetPosition() != 0);
		}
		void printTargetStatus(char lineNum = 2) {
			if (targetDetected()) {
				dsLCD->PrintfLine((DriverStationLCD::Line) lineNum, "Target at: %f", getTargetPosition());
				dsLCD->UpdateLCD();
			} else {
				printMessage("No Target Detected", lineNum);
			}
		...
		void runVisionCode() {
			printTargetStatus();
			if (targetDetected()) {
				if (getTargetPosition() > 0.40)
					turnRobot(0.2);
				else if (getTargetPosition() < -0.40)
					turnRobot(-0.2);
				else stopRobot();
			}
			else stopRobot();
